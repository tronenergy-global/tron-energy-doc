<template><div><h1 id="callback" tabindex="-1"><a class="header-anchor" href="#callback" aria-hidden="true">#</a> callback</h1>
<p>After the current commissioned energy is completed, it will try to send a message to notify the initiator. There are currently two places where you can set the callback address.</p>
<ul>
<li>
<ol>
<li>In <a href="https://%7Bdomain%7D/buyer/me" target="_blank" rel="noopener noreferrer">My<ExternalLinkIcon/></a> settings, set the default callback address</li>
</ol>
</li>
<li>
<ol start="2">
<li>Specify the callback_url parameter to overwrite the default callback address when placing an order.</li>
</ol>
</li>
</ul>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h2 id="respond-retry" tabindex="-1"><a class="header-anchor" href="#respond-retry" aria-hidden="true">#</a> Respond &amp; Retry</h2>
<p>The response body is as follows</p>
...
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>If you receive the callback information, you only need to return <code v-pre>HTTP 200</code>, which means that the message has been processed and this callback will no longer be sent. Otherwise, it will try to send repeatedly until the expected response is obtained.
The current retry frequency is <code v-pre>15s, 15s, 30s, 180s, 600s, 1200s, 1800s</code>. If there is no response, it will not be sent again.</p>
<h2 id="verification" tabindex="-1"><a class="header-anchor" href="#verification" aria-hidden="true">#</a> Verification</h2>
<p>When the server sends a callback message, it will use the order API to sign. The receiver needs to verify the signature and then perform logical processing, otherwise unnecessary losses may be caused. The server's signature process and order initiation [signature](/general/ sign.md), the receiver needs to use the timestamp in the callback request for encryption, and verify whether the <code v-pre>SIGNATURE</code> of the callback is consistent.</p>
