<template><div><h2 id="основная-информация" tabindex="-1"><a class="header-anchor" href="#основная-информация" aria-hidden="true">#</a> Основная информация</h2>
<p>Если ваша задача заключается в сборе средств с адресов для зачисления на горячий/холодный кошелек, стандартные энергетические интерфейсы могут оказаться недостаточными. Поэтому мы разработали специальный интерфейс для коллекции средств, который упрощает данный процесс.</p>
<h2 id="адрес-api" tabindex="-1"><a class="header-anchor" href="#адрес-api" aria-hidden="true">#</a> Адрес API</h2>
<div class="language-text line-numbers-mode" data-ext="text"><pre v-pre class="language-text"><code>POST /api/v1/frontend/order/collect-manual
...
<td>int</td>
<td>Да</td>
<td>Сценарий коллекции</td>
<td>0-автоматическое определение</td>
</tr>
<tr>
...
<td>string</td>
<td>Нет</td>
<td>Уведомление о успешной/неуспешной делегации будет отправлено на этот адрес</td>
...
<td>string</td>
<td>Нет</td>
<td>Этот параметр будет передан в обратном вызове для облегчения ассоциации заказа</td>
<td>S202302320004</td>
</tr>
</tbody>
</table>
...
<div class="language-json line-numbers-mode" data-ext="json"><pre v-pre class="language-json"><code><span class="token punctuation">{</span>
    ...
    <span class="token property">"scene"</span><span class="token operator">:</span> <span class="token number">0</span><span class="token punctuation">,</span>
    <span class="token property">"amount"</span><span class="token operator">:</span> <span class="token number">4000000</span><span class="token punctuation">,</span>
...
<td>serial</td>
<td>string</td>
<td>Внутренний номер заказа</td>
<td></td>
</tr>
<tr>
<td>scene</td>
<td>int</td>
<td>0 означает автоматическое определение, будет возвращен фактически выполненный сценарий</td>
<td></td>
</tr>
<tr>
<td>amount</td>
<td>int</td>
<td>Сумма заказа, единица измерения sun, для перевода в TRX нужно разделить на 1000000</td>
<td></td>
</tr>
<tr>
<td>balance</td>
...
<ul>
<li>После размещения заказа, чтобы узнать состояние выполнения заказа, вы можете использовать <RouterLink to="/ru/api/order-query.html">активный запрос</RouterLink> или <RouterLink to="/ru/general/callback.html">пассивный обратный вызов</RouterLink></li>
</ul>
</div>
...
URL <span class="token operator">=</span> <span class="token string">"https://{domain}/api/v1/frontend/order/collect-manual"</span>
data <span class="token operator">=</span> <span class="token punctuation">{</span>
   <span class="token string">'scene'</span><span class="token punctuation">:</span> <span class="token number">2</span><span class="token punctuation">,</span>
...
<span class="token variable">$data</span> <span class="token operator">=</span> <span class="token punctuation">[</span>
    <span class="token string single-quoted-string">'scene'</span> <span class="token operator">=></span> <span class="token number">2</span><span class="token punctuation">,</span>
...
<span class="token variable">$result</span> <span class="token operator">=</span> <span class="token function">file_get_contents</span><span class="token punctuation">(</span><span class="token string double-quoted-string">"https://{domain}/api/v1/frontend/order/collect-manual"</span><span class="token punctuation">,</span> <span class="token constant boolean">false</span><span class="token punctuation">,</span> <span class="token variable">$context</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
...
<span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">Main</span> <span class="token punctuation">{</span>
    ...
    <span class="token keyword">public</span> <span class="token keyword">static</span> <span class="token keyword">void</span> <span class="token function">main</span><span class="token punctuation">(</span><span class="token class-name">String</span><span class="token punctuation">[</span><span class="token punctuation">]</span> args<span class="token punctuation">)</span> <span class="token keyword">throws</span> <span class="token class-name">Exception</span> <span class="token punctuation">{</span>
        ...
        data<span class="token punctuation">.</span><span class="token function">put</span><span class="token punctuation">(</span><span class="token string">"scene"</span><span class="token punctuation">,</span> <span class="token number">2</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
        ...
        <span class="token class-name">Request</span> request <span class="token operator">=</span> <span class="token keyword">new</span> <span class="token class-name">Request<span class="token punctuation">.</span>Builder</span><span class="token punctuation">(</span><span class="token punctuation">)</span>
                <span class="token punctuation">.</span><span class="token function">url</span><span class="token punctuation">(</span><span class="token string">"https://{domain}/api/v1/frontend/order/collect-manual"</span><span class="token punctuation">)</span>
...
<span class="token keyword">type</span> Data <span class="token keyword">struct</span> <span class="token punctuation">{</span>
	Scene         <span class="token builtin">string</span> <span class="token string">`json:"scene"`</span>
...
<span class="token keyword">func</span> <span class="token function">main</span><span class="token punctuation">(</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
	...
	data <span class="token operator">:=</span> Data<span class="token punctuation">{</span>
		Scene<span class="token punctuation">:</span>   <span class="token number">2</span><span class="token punctuation">,</span>
...
<span class="token assign-left variable">URL</span><span class="token operator">=</span><span class="token string">"https://{domain}/api/v1/frontend/order/collect-manual"</span>
...
<span class="token keyword">const</span> <span class="token constant">URL</span> <span class="token operator">=</span> <span class="token string">"https://{domain}/api/v1/frontend/order/collect-manual"</span><span class="token punctuation">;</span>
<span class="token keyword">const</span> data <span class="token operator">=</span> <span class="token punctuation">{</span>
    <span class="token literal-property property">scene</span><span class="token operator">:</span> <span class="token number">2</span><span class="token punctuation">,</span>
