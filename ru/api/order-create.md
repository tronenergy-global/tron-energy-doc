...
| -------- | -------- | -------- | -------- | -------- |
| energy_amount   | int   |  Да  |  Необходимое количество энергии | 32000   |
...
| callback_url | string | Нет | Уведомление будет отправлено на этот адрес в случае успешного выполнения заказа | https://mydomain.com/callback |
| out_trade_no | string | Нет | Информация для удобства сопоставления заказов при получении уведомления | S202302320004 |
...
| amount | int | Сумма заказа, единица измерения sun |  |
| balance | int | Баланс, единица измерения sun, для перевода в TRX нужно разделить на 1000000 |  |

::: tip

- После оформления заказа для проверки статуса выполнения вы можете использовать [Активный запрос](/ru/api/order-query.md) или [Пассивное уведомление](/ru/general/callback.md)
  :::
